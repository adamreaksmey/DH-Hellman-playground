...
export class RegistrationClient {
  ...
  // State (would normally persist securely)
  deviceId?: string;
  ...
  async registerDevice(deviceName?: string): Promise<DeviceRegistrationResponse> {
    ...
    const { data } = await this.axios.post<DeviceRegistrationResponse>('/api/v1/device/register', payload);
  ...
  async fullRegistrationFlow(phoneNumber: string, otp: string, profile?: ProfileSetupRequest): Promise<AuthResponse> {
