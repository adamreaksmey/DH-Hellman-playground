...
async function run() {
  const jsonMinified = JSON.stringify(jsonData);
  const jsonPretty   = JSON.stringify(jsonData, null, 2);
  const jsonSize     = Buffer.byteLength(jsonPretty, 'utf8');
  const jsonMinSize  = Buffer.byteLength(jsonMinified, 'utf8');

  const packed    = pack(jsonData);
  const unpacked  = unpack(packed);
  const msgpacked = Buffer.from(encode(jsonData));

  const pct = (n: number, base: number) => ((1 - n / base) * 100).toFixed(1) + '%';

  console.log('=== Size comparison ===');
  console.log(`JSON (pretty):     ${jsonSize} bytes`);
  console.log(`JSON (minified):   ${jsonMinSize} bytes`);
  console.log(`MessagePack:       ${msgpacked.length} bytes  (${pct(msgpacked.length, jsonMinSize)} vs minified)`);
  console.log(`Custom binary:     ${packed.length} bytes   (${pct(packed.length, jsonMinSize)} vs minified)`);

  console.log('');
  console.log('=== Round-trip test ===');
  console.log(`Custom binary: ${deepEqual(jsonData, unpacked)}`);

  const msgUnpacked = decode(msgpacked) as ChatMessage;
  console.log(`MessagePack:   ${deepEqual(jsonData, msgUnpacked)}`);
}
